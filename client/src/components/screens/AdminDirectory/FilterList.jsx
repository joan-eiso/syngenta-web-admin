...
import { sortAndFilterAdministrators } from "../../../redux/filter/duck";
...
function FilterList({ handleClose }) {
  ...
  return (
    <div className={classes.root}>
      ...
      <Formik
        initialValues={{
          order: administratorSortingRule,
          status: administratorFilters["status"]
        }}
        onSubmit={(values) => {
          let filters = {
            status: values.status
          }
          dispatch(sortAndFilterAdministrators(values.order, filters, administrators));
          handleClose();
        }}
      >
        {({ values }) => (
